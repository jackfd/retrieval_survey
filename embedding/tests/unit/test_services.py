...
def test_process_doc_flushes_multiple_batches(monkeypatch: pytest.MonkeyPatch):
    ...
    selector = Mock()
    selector.top_n = 3
    selector.build_candidates.side_effect = [
        [{"order": 1, "text": "d1-c1"}, {"order": 2, "text": "d1-c2"}],
        [{"order": 1, "text": "d2-c1"}],
        [{"order": 1, "text": "d3-c1"}, {"order": 2, "text": "d3-c2"}],
    ]
    selector.select_from_embeddings.side_effect = [
    ...
    monkeypatch.setattr(document_service, "ChunkSelector", lambda: selector)

    output = Mock()

    document_service.process_doc(embedding, 2, Path("docs.jsonl"), output)
    ...
    assert embedding.encode.call_args_list[1].args[0] == ["d2-c1", "d3-c1"]
    assert embedding.encode.call_args_list[2].args[0] == ["d3-c2"]
    ...
    assert selector.select_from_embeddings.call_args_list[0].args[0] == "d1"
    assert selector.select_from_embeddings.call_args_list[1].args[0] == "d2"
    assert selector.select_from_embeddings.call_args_list[2].args[0] == "d3"
    assert [
        item["text"]
        for item in selector.select_from_embeddings.call_args_list[1].args[1]
    ] == ["d2-c1"]
    assert [
        item["text"]
        for item in selector.select_from_embeddings.call_args_list[2].args[1]
    ] == ["d3-c1", "d3-c2"]
    assert selector.select_from_embeddings.call_args_list[1].args[2].shape == (1, 2)
    assert selector.select_from_embeddings.call_args_list[2].args[2].shape == (2, 2)
...
def test_process_doc_raises_when_no_chunks(monkeypatch: pytest.MonkeyPatch):
    ...
    with pytest.raises(ProcessingError, match="No documents found"):
        document_service.process_doc(Mock(), 2, Path("docs.jsonl"), output)
...
def test_process_doc_waits_for_full_doc_before_selecting(
    ...
    embedding.encode.side_effect = [
        ...
        np.array([[0.0, 1.0], [0.2, 0.8]], dtype=np.float32),
    ]
    ...
    assert embedding.encode.call_args_list[0].args[0] == ["d1-c1", "d1-c2"]
    assert embedding.encode.call_args_list[1].args[0] == ["d1-c3", "d2-c1"]
    assert selector.select_from_embeddings.call_args_list[0].args[0] == "d1"
    assert selector.select_from_embeddings.call_args_list[0].args[2].shape == (3, 2)
    assert selector.select_from_embeddings.call_args_list[1].args[0] == "d2"
...
def test_process_doc_wraps_embedding_errors(monkeypatch: pytest.MonkeyPatch):
    ...
    selector = Mock()
    selector.top_n = 3
    selector.build_candidates.return_value = [{"order": 1, "text": "d1-c1"}]
    embedding = Mock()
    ...
    with pytest.raises(
        ...
        document_service.process_doc(embedding, 2, Path("docs.jsonl"), Mock())
...
def test_process_query_encodes_and_writes(monkeypatch: pytest.MonkeyPatch):
    ...
    query_service.process_query(embedding, 2, Path("queries.jsonl"), output)
    ...
    assert embedding.encode.call_args_list[1].kwargs == {"is_query": True}
...
def test_process_query_flushes_single_final_partial_batch(
    ...
    query_service.process_query(embedding, 8, Path("queries.jsonl"), output)
...
def test_process_query_rejects_empty_query_fields(monkeypatch: pytest.MonkeyPatch):
    ...
    with pytest.raises(ProcessingError):
        query_service.process_query(Mock(), 2, Path("queries.jsonl"), Mock())
...
def test_process_query_wraps_encoder_errors(monkeypatch: pytest.MonkeyPatch):
    ...
    with pytest.raises(
        ...
        query_service.process_query(embedding, 4, Path("queries.jsonl"), Mock())
...
def test_process_query_raises_when_encoder_returns_wrong_batch_size(
    ...
    with pytest.raises(
        ...
        query_service.process_query(embedding, 2, Path("queries.jsonl"), Mock())
