...
usage() {
  cat <<'EOF'
Usage:
  run_st.sh --dataset-path <path> [--source-config <path>] [--generated-config <path>]
...
Options:
  ...
  --generated-config  Optional. Generated filtered config path. Default: ./model_config.st.yaml
EOF
}

DATASET_PATH=""
SOURCE_CONFIG="${SOURCE_CONFIG:-embedding/model_config.yaml}"
GENERATED_CONFIG="${GENERATED_CONFIG:-./model_config.st.yaml}"
