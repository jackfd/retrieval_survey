...
from embed_pipe.domain.errors import EmbeddingGenerationError, ModelLoadError
...
def ensure_embedding_shape(vectors: np.ndarray, expected_dim: int) -> np.ndarray:
    ...
    if vectors.ndim != 2:
        ...
        raise EmbeddingGenerationError("Expected 2D embeddings, got shape=%s" % (vectors.shape,))
    if vectors.shape[1] != expected_dim:
        ...
        raise EmbeddingGenerationError(
            "Embedding dimension mismatch, expected=%s, got=%s" % (expected_dim, vectors.shape[1])
        )
    return vectors
...
class LocalEmbeddingStrategy(BaseEmbeddingStrategy):
    ...
    def _build_local_encoder(self, model: ModelConfig):
        ...
        raise ModelLoadError("Unsupported local provider=%r" % model.provider)
